const lunr = require('lunr')
const navigationConfig = require('../../config/navigation.json')
const catalogue = require('../../lib/catalogue.js')

module.exports = function lunrPlugin() {
    return (files, metalsmith, done) => {
        const outputPath = 'search-index.json'
<<<<<<< HEAD

        const includedSections = navigationConfig
            .filter(section => section.includeInSearch)
            .map(section => section.label)

        const documents =
            Object.keys(files)
            // Filter out any non html files
            .filter(path => path.endsWith('.html'))
            // Filter out anything not in an included section
            // .filter(path => {
            //   const section = files[path].section
            //   return includedSections.includes(section)
            // })
            .map(path => {
                const file = files[path]
                const documentPath = file.path || path
                return {
                    path: documentPath,
                    title: file.title,
                    section: file.section,
                    aliases: file.aliases === null ? undefined : file.aliases
                }
            })

=======
        const additionalSearch = catalogue.getList

        const includedSections = navigationConfig
            .filter(section => section.includeInSearch)
            .map(section => section.label)

        const documents =
            Object.keys(files)
            // Filter out any non html files
            .filter(path => path.endsWith('.html'))
            // Filter out anything not in an included section
            .filter(path => {
                const section = files[path].section
                return includedSections.includes(section)
            })
            .map(path => {
                const file = files[path]
                const documentPath = file.path || path
                var sup_aliases = ""

                additionalSearch.forEach(entry => {
                    if (entry.path === documentPath) {
                        sup_aliases = entry.label + ", " + entry.aliases.replace(" ", ", ")
                    }
                })

                var full_aliases = ""

                if (sup_aliases) {
                    full_aliases = file.aliases === null ? sup_aliases : file.aliases + " " + sup_aliases
                } else {
                    full_aliases = file.aliases === null ? undefined : file.aliases
                }

                return {
                    path: documentPath,
                    title: file.title,
                    section: file.section,
                    aliases: full_aliases
                }

            })

>>>>>>> master
        const pageHeadingDocuments =
            Object.keys(files)
            // Filter out any non html files
            .filter(path => path.endsWith('.html'))
            // Only include pages in the with show_page_nav: true
            .filter(path => {
                return files[path].show_page_nav
<<<<<<< HEAD
            })
            // Filter out files that don't have extracted headings
            .filter(path => files[path].headings)
            .map(path => {
                let file = files[path]
                let documentPath = file.path || path
                let headings = file.headings
                    // only use <h2>s
                    .filter(heading => heading.depth === 2)
                    .map(heading => {
                        return {
                            path: `${documentPath}/#${heading.url}`,
                            title: heading.text,
                            page: file.title,
                            section: file.section
                        }
                    })
                return headings
            })
=======
            })
            // Filter out files that don't have extracted headings
            .filter(path => files[path].headings)
            .map(path => {
                let file = files[path]
                let documentPath = file.path || path
                let headings = file.headings
                var sup_aliases = ""

                // only use <h2>s
                .filter(heading => heading.depth === 2)
                    .map(heading => {

                        additionalSearch.forEach(entry => {
                            if (entry.path === documentPath) {
                                sup_aliases = entry.label + ", " + entry.aliases.replace(" ", ", ")
                            }
                        })

                        var full_aliases = ""

                        if (sup_aliases) {
                            full_aliases = file.aliases === null ? sup_aliases : file.aliases + " " + sup_aliases
                        } else {
                            full_aliases = file.aliases === null ? undefined : file.aliases
                        }

                        return {
                            path: `${documentPath}/#${heading.url}`,
                            title: heading.text,
                            page: file.title,
                            section: file.section,
                            aliases: full_aliases
                        }
                    })
                return headings
            })
>>>>>>> master
            // we need a flat array
            .reduce((arr, curr) => {
                return arr.concat(curr)
            }, [])

<<<<<<< HEAD
=======



>>>>>>> master
        // The search index only contains what's needed to match and identify a
        // document, but won't give us back anything other than the document's
        // identifier (`ref`).
        //
        // This store then allows us to lookup the information about the document
        // that we can use to present the result.
        let store = {}

        const index = lunr(function() {
            this.ref('path')
            this.field('title', { boost: 100 })
            this.field('aliases', { boost: 10 })

            // Stemming (reducing words to their stem e.g. fishing to fish) is not
            // useful in an 'autocomplete' scenario, where we search using a prefix
            // and users get instant feedback. It can also cause confusion as
            // partially completed words will not be stemmed, e.g. 'fishi' – which can
            // cause search results to disappear and re-appear again.

            // Disable stemming of documents when generating the index
            this.pipeline.remove(lunr.stemmer)
                // Disable stemming of search terms run against this index
            this.searchPipeline.remove(lunr.stemmer)
<<<<<<< HEAD
=======

>>>>>>> master
            documents.forEach(doc => {
                    store[doc.path] = {
                        title: doc.title,
                        path: doc.path,
<<<<<<< HEAD
                        section: doc.section === undefined ? 'NICS Architecture Portal' : doc.section,
=======
                        section: doc.section,
>>>>>>> master
                        aliases: doc.aliases
                    }
                    this.add(doc)
                })
                // store all headings into the Lunr store
            pageHeadingDocuments.forEach(doc => {
                let separator = '<span class="app-site-search__separator" aria-hidden="true">›</span>'
                store[doc.path] = {
                    title: doc.title,
                    path: doc.path,
                    section: `${doc.section}${separator}${doc.page}`
                }
                this.add(doc)
            })
<<<<<<< HEAD
=======

            // additionalSearch.forEach(doc => {
            //     store[doc.path] = {
            //         title: doc.label,
            //         path: doc.path,
            //         section: doc.section,
            //         aliases: doc.aliases
            //     }

            //     this.add(doc)
            // })
>>>>>>> master
        })

        // Add search index to metalsmith's file object, so that it will write
        // the file out as part of its build
        files[outputPath] = {
            contents: JSON.stringify({
                index,
                store
            })
        }

        done()
    }
}